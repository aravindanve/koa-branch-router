...
function lookup(root, method, path, initialParams = {}) {
  ...
  while (stack.length) {
    ...
    if (node.type === TYPE.FRAGMENT_NODE) {
      ...
      caseSensitive =
        node.caseSensitive !== undefined ? node.caseSensitive : caseSensitive;
      strict = node.strict !== undefined ? node.strict : strict;
    ...
    if (node.type === TYPE.FRAGMENT_END) {
      ...
      // push boundary end delimiter
      if (node.isBoundary) {
        chain.push({
          type: TYPE.BOUNDARY_END,
        });
      }
      continue;
    }
