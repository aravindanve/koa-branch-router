...
class Router extends RouteFragment {
  ...
  layers(method, path, params) {
    ...
    const lastHandlerIndex = layers.reduce(
    ...
    return lastHandlerIndex !== -1 ? layers.slice(0, lastHandlerIndex + 1) : [];
  }
  ...
  routes() {
    ...
    return (ctx, next) => {
      ...
      // NOTE: do not remove duplicate slashes, users
      // should use an app level middleware to do that!
      const path = ctx.path;
